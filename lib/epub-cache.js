...
module.exports.init = function init(config) {
	console.log('initializing cache with config: ', config);
...
module.exports.cache = function cache (epubfile, cacheId, cb) {
	...
	/*
	console.log('arguments of CACHE:');
	console.log('epubfile = ', typeof(arguments[0]));
	console.log('epubfile = ', arguments[0]);
	console.log('cacheId = ', typeof(arguments[1]));
	console.log('cacheId = ', arguments[1]);
	console.log('cb = ', typeof(arguments[2]));
	*/
...
/*cacheid+'|'+epubData.easy.primaryID.value+'|'+epubData.easy.primaryID.scheme;//getUid();*/
		...
		for(file in files) {
			//console.log('processing :', files[file]);
